...
print("import work")
wandb.login(key="5e9d78805fc51d3056edd4b110cb9e0af6ca562f")
...
class SupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str, tokenizer: transformers.PreTrainedTokenizer):
        ...
        targets = [f"{example['response']}{tokenizer.eos_token}" for example in ds] + [f"{example['response']}{tokenizer.eos_token}" for example in list_data_dict]

        c = list(zip(sources, targets))
        import random       
        random.shuffle(c)
        sources, targets = zip(*c)
...
def train():
    ...
    model = transformers.AutoModelForCausalLM.from_pretrained(
        ...
        cache_dir="/nlp/scr/qinanyu/model_cache",
    )
